...
VIDEO_PATH = "part_13_KN6_cropped.mp4"
...
def is_person_on_vehicle(person_box, vehicle_boxes, track_id, frame):
    """Çok daha hassas araç üzerindeki kişi tespiti"""
    if not vehicle_boxes:  # Hiç araç yoksa, yaya olarak kabul et
        return False
    ...
    for vbox in vehicle_boxes:
        ...
        # 1. Çok yüksek IoU kontrolü (sadece gerçekten üst üste olanlar)
        ...
        # 2. Tam merkez nokta kontrolü - daha katı
        center_in_vehicle = (p_center_x > vx1 + v_width*0.2 and 
                            p_center_x < vx2 - v_width*0.2 and
                            p_center_y > vy1 + v_height*0.2 and 
                            p_center_y < vy2 - v_height*0.2)
        
        if center_in_vehicle:
            cv2.rectangle(frame, (int(vx1), int(vy1)), (int(vx2), int(vy2)), (0, 255, 255), 2)
            cv2.putText(frame, "CENTER_IN", (int(vx1), int(vy1) - 10), 
...
def is_pedestrian(track_id, current_position, frame_height):
    """Daha esnek yaya tespiti - sadece kesin araç sürücülerini filtrele"""
    if track_id not in track_history or len(track_history[track_id]['positions']) < 8:
        return True  # Yeterli veri yoksa yaya olarak kabul et
    
    positions = list(track_history[track_id]['positions'])
    
    # 1. Aşırı hız kontrolü - sadece çok hızlı olanları filtrele
    speed = calculate_speed(positions)
    if speed > SPEED_THRESHOLD:
        return False
    
    # 2. Çok düzgün hareket kontrolü - sadece aşırı düzgün olanları filtrele
    smoothness = calculate_movement_smoothness(positions)
    if smoothness > MOVEMENT_SMOOTHNESS_THRESHOLD:
        return False
    
    # 3. Pozisyon kontrolü - sadece çok yüksekte olanları filtrele
    avg_y = np.mean([pos[1] for pos in positions])
    height_ratio = avg_y / frame_height
    
    # Frame'in üst %30'unda olanları şüpheli say (daha esnek)
    if height_ratio < (1 - PEDESTRIAN_AREA_RATIO):
        return False
    
    return True  # Diğer tüm durumlar yaya olarak kabul edilir
